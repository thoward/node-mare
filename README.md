...
```javascript
    {
        // POST body contents
        data: ...,  // data member from the node-mare.server/task POST   
        type: ...,  // what type of task this is. eg. 'divide' or 'process'
        method: ... // data member
    }
```

It will return 201 Created with a ~broker/task/{id} location header on success or some error code on failure. 
